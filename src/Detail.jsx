...
import { C, font, PHASES, PHASE_INDEX, STATUS_CONFIG, fmt, fmtS, fmtD, sumPay, sumVal, btnS, imgUrl } from "./tokens.js";
  ...
  return(
    <div style={{border:`1px solid ${C.borderLight}`,borderRadius:8,overflow:"hidden",marginBottom:10}}>
      ...
      {open&&<div style={{borderTop:`1px solid ${C.borderLight}`}}>
        ...
        {isEditor&&<div style={{padding:"10px 18px",background:"#F7F9FB",borderTop:`1px solid ${C.borderLight}`}}><button onClick={e=>{e.stopPropagation();onAddPayment(contract);}} style={{padding:"7px 16px",borderRadius:6,border:"none",cursor:"pointer",background:C.teal,color:C.white,fontWeight:700,fontSize:11,fontFamily:font}}>+ Registrar pago</button></div>}
      </div>}
    </div>
  );
}
...
export default function ProjectDetail({project:p,onBack,onEdit,onAddAdvance,onAddContract,onAddPayment}){
  ...
  return(
    <div style={{maxWidth:840,margin:"0 auto"}}>
      ...
      <Section title="Información general" accent={sc.color} action={isEditor?<button onClick={onEdit} style={{background:"none",border:`1px solid ${C.border}`,borderRadius:6,padding:"5px 14px",fontSize:11,fontWeight:700,color:C.textSecondary,cursor:"pointer",fontFamily:font}}>✎ Editar</button>:null} style={{marginBottom:16}}>
        <div style={{padding:isMobile?"16px 16px 0":"24px 24px 0"}}><div style={{display:"flex",flexDirection:isMobile?"column":"row",gap:16,flexWrap:"wrap"}}>
          <div style={{flex:1,minWidth:isMobile?"auto":250}}>
            ...
            {p.sede&&<p style={{fontSize:11,color:C.textMuted,margin:"0 0 4px",fontWeight:600}}>📍 {p.sede}</p>}
          ...
          {p.fotoPrincipal&&
            <img src={imgUrl(p.fotoPrincipal, 600)} alt="foto principal" loading="eager" style={{width:isMobile?"100%":200,height:isMobile?200:160,objectFit:"contain",background:C.bg,borderRadius:8,flexShrink:0,display:"block",border:`1px solid ${C.border}`,order:isMobile?-1:0}}/>
          }
        </div></div>
      ...
      {tab==="timeline"&&<>{isEditor&&<button onClick={onAddAdvance} style={{padding:"10px 24px",borderRadius:6,border:"none",cursor:"pointer",background:C.navy,color:C.white,fontWeight:700,fontSize:12,fontFamily:font,marginBottom:16}}>+ Registrar avance</button>}
      ...
      {tab==="finance"&&<>
        ...
        {isEditor&&<button onClick={onAddContract} style={{padding:"10px 24px",borderRadius:6,border:"none",cursor:"pointer",background:C.teal,color:C.white,fontWeight:700,fontSize:12,fontFamily:font,marginBottom:16}}>+ Nuevo contrato / ODS</button>}
